...
export default function MobileStickyBar({ onOpenContact }: MobileStickyBarProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 sm:hidden bg-[#0b0c10]/95 backdrop-blur-xl border-t border-white/10 p-2.5 px-3 flex items-center gap-2 shadow-2xl font-sans">
      <a
        href="tel:022739919337"
        aria-label="New York Nails Horrem anrufen"
        className="flex-1 bg-white/5 hover:bg-white/10 text-white font-medium text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 transition-colors border border-white/10"
      >
      ...
      <button
        ...
        className="flex-1 bg-[#c5a059] hover:bg-[#dfb76c] text-[#0b0c10] font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
      >
